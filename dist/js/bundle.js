...
// todo削除用コンポーネント
_vue2.default.component('todo-delete', {
  props: ['index'],
  methods: {
  ...
  template: '<i class="fa fa-trash icon-trash" v-on:click="deleteItem" />'
});

// todo完了用コンポーネント
_vue2.default.component('todo-done', {
  props: ['index'],
  methods: {
  ...
  template: '<i class="fa fa-circle-thin icon-check" v-on:click="doneItem" />'
});

// todo編集用コンポーネント
_vue2.default.component('todo-edit', {
  props: ['item', 'index'],
  data: function data() {
    return {
      tempItem: {
...
_vue2.default.component('todo-list', {
  ...
  methods: {
    addItem: function addItem(id, text) {
      this.items.push({
        id: id,
        text: text,
        isDone: false,
    ...
    editItem: function editItem(index, text) {
      this.$nextTick(function () {
  ...
  computed: {
    searchItem: function searchItem() {
      var regexp = new RegExp('^' + this.searchWord + '[a-zA-Z0-9]*'); // 検索条件（前方一致）を定義
      var data = this.items; // 現在のオブジェクトを保存
      if (this.searchWord) {
        // 検索フォームに入力があった場合
        data = this.items.filter(function (item) {
          // 前方一致でヒットしたオブジェクトを配列に格納する
